    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    {
        std::cout << "[MODE] GCN " << std::endl;
        mpGCNextractorLeft = new GCNextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
    }
    else
    {
        std::cout << "[MODE] ORB " << std::endl;
        mpORBextractorLeft = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
    }
    
    if (getenv("USE_ORB") == nullptr && sensor==System::STEREO){
        //std::cout << "[INFO] creating GCN right extractor " << std::endl;
        mpGCNextractorRight = new GCNextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
    }
...
{
    //std::cout << "[GrabImageStereo] imLeft size: " << imRectLeft.cols << "x" << imRectLeft.rows << ", imRight size: " << imRectRight.cols << "x" << imRectRight.rows << std::endl;

    if (imRectLeft.empty() || imRectRight.empty()) {
        std::cerr << "[GrabImageStereo] ERROR: One or both input images are empty!" << std::endl;
        return cv::Mat();
    }
    ...
    //std::cerr << "[GrabImageStereo] Using GCNextractor in Frame" << std::endl;
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp)
{
    ...
    {
        // GCN
        mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpGCNextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    }
    else
    {
        // Orb
